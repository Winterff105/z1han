...
let homeScreenData = [
    ...
    { index: 30, type: 'app', name: 'MCP', iconClass: 'fas fa-plug', color: '#9FB7CA', appId: 'mcp-app' },
...
function initGrid() {
    ...
    // 强制添加 MCP 接入应用 (如果不存在)
    if (!homeScreenData.some(item => item.appId === 'mcp-app')) {
        const targetIndex = findFirstAvailableSlot(30, SLOTS_PER_PAGE * 2);
...
function checkAndShowUpdateModal() {
    const currentVersion = '1.4'; // 布局版本号
    ...
    if (savedVersion !== currentVersion) {
        ...
        content.innerHTML = `
            ...
            <p style="margin: 0 0 20px 0; color: #666; font-size: 14px; line-height: 1.5;">
                桌面布局已更新（第二页新增了“工作室”和“MCP 接入”应用）。<br>
                请点击下方按钮恢复默认布局以生效。
            </p>
        ...
        document.getElementById('confirm-reset-layout').onclick = () => {
            ...
            const defaultPage2 = [
                ...
                { index: 30, type: 'app', name: 'MCP', iconClass: 'fas fa-plug', color: '#9FB7CA', appId: 'mcp-app', _internalId: generateId() }
            ];
