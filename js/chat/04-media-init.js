...
const CHAT_WEB_LINK_CONTEXT_MAX_URLS_PER_MESSAGE = 3;
...
function formatWebLinkContextPayload(successResults = []) {
    ...
    successResults.forEach((item, index) => {
        ...
        const title = sanitizeWebLinkContextText(item && item.title, 220);
        const summary = sanitizeWebLinkContextText(item && item.summary, 900);
    ...
    const fetchPromise = (async () => {
        try {
            ...
            maybeSendWebLinkContextFailureNotice(
                contactId,
                `读取链接失败：${error && error.message ? error.message : '未知错误'}。你可以重试，或直接粘贴网页关键段落。`,
                signature
            );
            settledResult = {
                ok: false,
